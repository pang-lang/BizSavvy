...
function getFacebookService() {
  return OAuth2.createService('Facebook')
    ...
    .setClientId('2244546899222939')
    .setClientSecret('3bab8057d2daa67938e1afc52a0ccc63')
    .setCallbackFunction('handleCallback')
    .setPropertyStore(PropertiesService.getUserProperties())
    .setScope('email,public_profile,pages_manage_engagement,pages_manage_posts,pages_read_engagement,pages_show_list,instagram_basic,instagram_content_publish,instagram_manage_insights,business_management,read_insights,instagram_manage_insights')
...
function postToFacebookPage(pageId, accessToken, caption, imageUrls) {
  ...
  try {
    if (imageUrls.length === 0) {
      // If no images, post as a status update
      ...
      // If single image, post as a photo
      var payload = {
        'message': message,
        'url': imageUrls[0],
        'access_token': accessToken
      ...
      // If multiple images, use multi-photo post
...
function collectAnalytics() {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    var facebookAccessToken = getFacebookAccessToken();
    var instagramAccessToken = getInstagramAccessToken();
  ...
  var facebookPostSheet = spreadsheet.getSheetByName('FacebookPosts');
  var instagramPostSheet = spreadsheet.getSheetByName('InstagramPosts');
  var facebookOverallSheet = spreadsheet.getSheetByName('FacebookOverall');
  var instagramOverallSheet = spreadsheet.getSheetByName('InstagramOverall');

  ensureHeaders(facebookPostSheet, ['Date', 'Publish Date', 'Title', 'Post ID', 'Page ID', 'Impressions', 'Likes']);
  ensureHeaders(instagramPostSheet, ['Date', 'Publish Date', 'Title', 'Post ID', 'Account ID', 'Impressions', 'Engagement']);
  ensureHeaders(facebookOverallSheet, ['Date', 'Page ID', 'Impressions', 'Actions', 'Engagements', 'Fan Adds', 'Fan Removes', 'Page Views']);
  ensureHeaders(instagramOverallSheet, ['Date', 'Account ID', 'Impressions', 'Reach', 'Engaged Accounts', 'Profile Views']);

  // Batch process Facebook posts
  if (facebookPosts.length > 0) {
    var facebookPostAnalytics = batchGetFacebookAnalytics(facebookPosts);
    appendAnalytics(facebookPostSheet, facebookPostAnalytics);
  }

  if (instagramPosts.length > 0) {
    var instagramPostAnalytics = batchGetInstagramAnalytics(instagramPosts);
...
function createAnalyticsTrigger() {
  ...
  // Create a new trigger to run daily at 2 AM
...
function batchGetInstagramAnalytics(posts) {
  console.log("Starting batchGetInstagramAnalytics for", posts.length, "posts");
  var analytics = [];
  var accessToken = getInstagramAccessToken();
        ...
        if (result.code === 200) {
          ...
          if (data && data.length > 0) {
            analytics.push([
              ...
              data.find(d => d.name === 'total_interactions')?.values[0]?.value || 0
            ]);
          } else {
            console.log("No data available for post", posts[index].id);
            analytics.push([
...
function getFacebookOverallMetrics(pageId, accessToken) {
  console.log("Fetching overall Facebook metrics for page:", pageId);
  var metrics = [];
  var url = `https://graph.facebook.com/v20.0/${pageId}/insights?metric=page_impressions,page_actions_post_reactions_like_total,page_fan_adds,page_fan_removes,page_views_total`;

  try {
    var response = UrlFetchApp.fetch(url + `&access_token=${accessToken}`, {
      muteHttpExceptions: true
    });

    var responseCode = response.getResponseCode();
    var responseBody = response.getContentText();
    console.log("Facebook Overall API Response Code:", responseCode);
    console.log("Facebook Overall API Response Body:", responseBody);

    if (responseCode === 200) {
      var data = JSON.parse(responseBody).data;
      console.log("Facebook Overall data for page", pageId, ":", JSON.stringify(data));
      metrics.push([
        new Date(),
        pageId,
        data.find(d => d.name === 'page_impressions')?.values[0]?.value || 0,
        data.find(d => d.name === 'page_actions_post_reactions_like_total')?.values[0]?.value || 0,
        data.find(d => d.name === 'page_fan_adds')?.values[0]?.value || 0,
        data.find(d => d.name === 'page_fan_removes')?.values[0]?.value || 0,
        data.find(d => d.name === 'page_views_total')?.values[0]?.value || 0
      ]);
    } else {
      throw new Error(`Facebook Overall API returned status code ${responseCode}: ${responseBody}`);
    }
  } catch (error) {
    console.error("Error in getFacebookOverallMetrics:", error);
  }

  return metrics;
}

function getInstagramOverallMetrics(accountId, accessToken) {
  console.log("Fetching overall Instagram metrics for account:", accountId);
  var metrics = [];
  var url = `https://graph.facebook.com/v20.0/${accountId}/insights?metric=impressions,reach,engaged_users,profile_views`;
  ...
  try {
    ...
    var responseCode = response.getResponseCode();
    var responseBody = response.getContentText();
    console.log("Instagram Overall API Response Code:", responseCode);
    console.log("Instagram Overall API Response Body:", responseBody);

    if (responseCode === 200) {
      var data = JSON.parse(responseBody).data;
      console.log("Instagram Overall data for account", accountId, ":", JSON.stringify(data));
      metrics.push([
        new Date(),
        accountId,
        data.find(d => d.name === 'impressions')?.values[0]?.value || 0,
        data.find(d => d.name === 'reach')?.values[0]?.value || 0,
        data.find(d => d.name === 'engaged_users')?.values[0]?.value || 0,
        data.find(d => d.name === 'profile_views')?.values[0]?.value || 0
      ]);
    } else {
      throw new Error(`Instagram Overall API returned status code ${responseCode}: ${responseBody}`);
    }
  } catch (error) {
    console.error("Error in getInstagramOverallMetrics:", error);
  }
...
function getFacebookAccessToken() {
  var scriptProperties = PropertiesService.getScriptProperties();
  return scriptProperties.getProperty('FACEBOOK_ACCESS_TOKEN');
}

function getInstagramAccessToken() {
  var scriptProperties = PropertiesService.getScriptProperties();
  return scriptProperties.getProperty('INSTAGRAM_ACCESS_TOKEN');
}

function setProperties() {
  var scriptProperties = PropertiesService.getScriptProperties();
  scriptProperties.setProperty('FACEBOOK_ACCESS_TOKEN', 'your-facebook-access-token');
  scriptProperties.setProperty('INSTAGRAM_ACCESS_TOKEN', 'your-instagram-access-token');
  scriptProperties.setProperty('spreadsheetUrl', 'your-spreadsheet-url');
}

function refreshFacebookToken() {
  var facebookService = getFacebookService();
  if (facebookService.hasAccess()) {
    var accessToken = facebookService.getAccessToken();
    updateTokenInSpreadsheet('Facebook', accessToken);
    return accessToken;
  } else {
    throw new Error('Failed to refresh Facebook token. Re-authentication required.');
  }
}
